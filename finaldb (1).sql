...
--
-- Temporary view structure for view `libridisponibili`
--

DROP TABLE IF EXISTS `libridisponibili`;
/*!50001 DROP VIEW IF EXISTS `libridisponibili`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `libridisponibili` AS SELECT 
 1 AS `CodiceLibro`,
 1 AS `Titolo`,
 1 AS `Edizione`,
 1 AS `Anno`,
...
--
-- Final view structure for view `libridisponibili`
--
...
/*!50001 VIEW `libridisponibili` AS select `l`.`CodiceLibro` AS `CodiceLibro`,`l`.`Nome` AS `Titolo`,`l`.`Edizione` AS `Edizione`,`l`.`Anno` AS `Anno`,`l`.`CostoPrestito` AS `CostoPrestito`,`l`.`Genere` AS `Genere`,`a`.`Nome` AS `Autore`,`c`.`Posizione` AS `Posizione`,`s`.`Genere` AS `Settore` from (((`libri` `l` join `collocati` `c` on((`l`.`CodiceLibro` = `c`.`Libri_CodiceLibro`))) join `autori` `a` on((`l`.`Autori_idAutori` = `a`.`idAutori`))) join `settori` `s` on((`c`.`Settori_idSettore` = `s`.`idSettore`))) where (`c`.`Disponibilita` = true) */;
...
-- Dump completed on 2025-07-23 13:17:34
